...
<?php
     ...
     if (isset($_SERVER['HTTP_REFERER']) == true) {
          if (limpa_pro($_SESSION['wrknompro']) != limpa_pro($_SERVER['HTTP_REFERER'])) {
               ...
               $ret = gravar_log(6, "Entrada na página de manutenção de empresa Pallas.41 - MyLogBox");  
          }
     }
          ...
          <div class="qua-0">
               <div class="row">
                    <div class="qua-2 col-md-12 text-left">
                         <span>Manutenção de Parâmetros</span>
                    </div>
               </div>
...
function empresa_exi($cod) {
     $cod = 1;
     include_once "dados.php";
     $nro = quantidade_reg('Select idempresa from tb_empresa where idempresa = 1', $men, $reg);     
     if ($nro == 0) {
...
function ler_empresa(&$cha, &$cgc, &$des, &$fan, &$ema, &$sta, &$tel, &$cep, &$end, &$num, &$com, &$bai, &$cid, &$est, &$ins, &$cel, &$con, &$sit) {
     include_once "dados.php";
     $nro = quantidade_reg("Select * from tb_empresa where idempresa = " . $cha, $men, $reg);     
     if ($nro == 0 || $reg == false) {
