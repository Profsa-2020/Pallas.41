...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
     ...
     <title>MyLogBox - Controle de Estoques de compras de clientes nos EUA - Grupos</title>
</head>
...
<?php
     ...
     if (isset($_SERVER['HTTP_REFERER']) == true) {
          if (limpa_pro($_SESSION['wrknompro']) != limpa_pro($_SERVER['HTTP_REFERER'])) {
               ...
               $ret = gravar_log(6,"Entrada na página de manutenção de grupos de produtos do sistema Pallas.41 - MyLogBox");  
          }
     }
     ...
     if ($_SESSION['wrkopereg'] >= 2) {
          if (isset($_REQUEST['salvar']) == false) { 
               ...
               $ret = ler_grupo($cha, $des, $sta); 
          }
     }
     if ($_SESSION['wrkopereg'] == 3) { 
          $bot = 'Deletar'; 
          $per = ' onclick="return confirm(\'Confirma exclusão de grupo de produto informado em tela ?\')" ';
     }
 ...
 if (isset($_REQUEST['salvar']) == true) {
      if ($_SESSION['wrkopereg'] == 1) {
           ...
           if ($sta == 0) {
                $ret = incluir_gru();
                $cod = ultimo_cod();
                $ret = gravar_log(11,"Inclusão de novo grupo de produtos: " . $des); 
      ...
      if ($_SESSION['wrkopereg'] == 2) {
           ...
           if ($sta == 0) {
                $ret = alterar_gru();
                $cod = ultimo_cod(); 
                $ret = gravar_log(12,"Alteração de grupo de produtos cadastrado: " . $des); 
                $des = ''; $sta = 00;  $_SESSION['wrkopereg'] = 1; $_SESSION['wrkcodreg'] = 0;
           }
      }
      if ($_SESSION['wrkopereg'] == 3) {
           ...
           $cod = ultimo_cod(); 
           $ret = gravar_log(13,"Exclusão de grupo de produtos cadastrado: " . $des); 
           $des = ''; $sta = 00;  $_SESSION['wrkopereg'] = 1; $_SESSION['wrkcodreg'] = 0;
      }
}
?>
...
<body id="box00">
<h1 class="cab-0">MyLogBox - Manutenção Grupos de Produtos  - Controle de Estoques - Profsa Informática</h1>
     <div class="row">
          <div class="col-md-12">
          ...
          <div class="qua-0">
               <div class="row qua-2">
                    <div class="col-md-11 text-left">
                         <span>Manutenção de Grupos de Produtos</span>
                    </div>
                         ...
                         <form name="frmTelNov" action="man-grupo.php?ope=1&cod=0" method="POST">
                              <div class="text-center">
                                   <button type="submit" class="bot-2" id="nov" name="novo"
                                        title="Mostra campos para criar novo grupo de produtos no sistema"><i
               ...
               <form class="tel-1" name="frmTelMan" action="" method="POST">
                    <div class="form-row">
                         ...
                         <div class="col-md-8">
                              <label>Descrição do Grupo de Produto</label>
                         ...
                         <table class="table table-sm table-striped">
                              <thead>
                                   <tr>
                                        ...
                                        <th scope="col">Descrição do Grupo de Produto</th>
...
if ($_SESSION['wrkopereg'] == 1 && $_SESSION['wrkcodreg'] == $cod) {
     exit('<script>location.href = "man-grupo.php?ope=1&cod=0"</script>');
}
...
function consiste_gru() {
     ...
     if (trim($_REQUEST['des']) == "") {
          echo '<script>alert("Descrição do grupo de produto não pode estar em branco");</script>';
          return 1;
     }
     return $sta;
 }
...
function carrega_gru() {
     include_once "dados.php";
     $com = "Select * from tb_grupo where gruempresa  = " . $_SESSION['wrkcodemp'] . " and grutiporeg = 1 order by grudescricao, idgrupo";
     $nro = carrega_tab($com, $reg);
     foreach ($reg as $lin) {
          $txt =  '<tr>';
          $txt .= '<td class="bot-3 text-center"><a href="man-grupo.php?ope=2&cod=' . $lin['idgrupo'] . '" title="Efetua alteração do registro informado na linha"><i class="bot-1a large material-icons">healing</i></a></td>';
          $txt .= '<td class="lit-d bot-3 text-center"><a href="man-grupo.php?ope=3&cod=' . $lin['idgrupo'] . '" title="Efetua exclusão do registro informado na linha"><i class="bot-1e large material-icons">delete_forever</i></a></td>';
...
function ler_grupo(&$cha, &$des, &$sta) {
     include_once "dados.php";
     $sql = "Select * from tb_grupo where idgrupo = " . $cha;
     $nro = carrega_reg($sql, $lin);
     if ($nro == 0) {
          echo '<script>alert("Código do grupo informado não cadastrada no sistema");</script>';
     } else {
          $cha = $lin['idgrupo'];
